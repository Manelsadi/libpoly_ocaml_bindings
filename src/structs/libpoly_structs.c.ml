...
module DyadicInterval = struct
  ...
  [%%c
  external a : interval_:t ptr -> lp_dyadic_rational_t ptr
    = {|
                    return &$interval_->a;
                    |}]

  [%%c
  external b : interval_:t ptr -> lp_dyadic_rational_t ptr
    = {|
                    return &$interval_->b;
                    |}]

  [%%c
  external a_open : interval_:t ptr -> size_t
    = {|
                    return $interval_->a_open;
                    |}]

  [%%c
  external b_open : interval_:t ptr -> size_t
    = {|
                    return $interval_->b_open;
                    |}]

  [%%c
  external is_point : interval_:t ptr -> size_t
    = {|
